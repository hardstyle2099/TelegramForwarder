...
from enums.enums import ForwardMode, MessageMode, PreviewMode, AddMode, HandleMode, UserForwardMode
...
RULE_SETTINGS = {
    ...
    'user_copy_mode': {
        'display_name': '用户模式发送方式',
        'values': {
            UserForwardMode.FORWARD: '原生转发(显示来源)',
            UserForwardMode.COPY:    '复制发送(隐藏来源)',
        },
        'toggle_action': 'toggle_user_copy_mode',
        'toggle_func': lambda current: UserForwardMode.COPY if current == UserForwardMode.FORWARD else UserForwardMode.FORWARD
    },
    'is_preview': {
        'display_name': '预览模式',
        'values': {
...
async def create_buttons(rule):
    ...
    try:
        ...
        if rule.use_bot:  # 只在使用机器人时显示这些设置
            ...
            buttons.append([
                Button.inline(
                    f"📤 发送方式: {RULE_SETTINGS['user_copy_mode']['values'][rule.user_copy_mode]}",
                    f"toggle_user_copy_mode:{rule.id}"
                )
            ])
